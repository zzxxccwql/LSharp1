...
namespace Marksman
{
    internal class Jinx : Champion
    {
        ...
        public override bool HarassMenu(Menu config)
        {
            ...
            config.AddItem(
                new MenuItem("UseQTH", "Q (切换!)").SetValue(new KeyBind("T".ToCharArray()[0], KeyBindType.Toggle)));
            config.AddItem(new MenuItem("UseQTHM", "Q 蓝量.").SetValue(new Slider(50, 100, 0)));

            config.AddItem(
                new MenuItem("UseWTH", "W (切换!)").SetValue(new KeyBind("Y".ToCharArray()[0], KeyBindType.Toggle)));
            config.AddItem(new MenuItem("UseWTHM", "W 蓝量.").SetValue(new Slider(50, 100, 0)));
            return true;
        }

        public override bool LaneClearMenu(Menu config)
        {
            config.AddItem(new MenuItem("SwapQ" + Id, "总是用机枪").SetValue(false));
            return true;
        }

        public override bool MiscMenu(Menu config)
        {
            var xQMenu = new Menu("Q 设置", "QSettings");
            {
                xQMenu.AddItem(
                    new MenuItem("SwapAOE", "互换Q 如果击中敌人数 >=").SetValue(
                        new Slider(2, 0, 5)));
                xQMenu.AddItem(new MenuItem("SwapDistance", "对调Q的距离").SetValue(true));
                xQMenu.AddItem(new MenuItem("Swap2Mini", "没有敌人只使用机枪").SetValue(true));
                config.AddSubMenu(xQMenu);
            }

            var xWMenu = new Menu("W 设置", "WSettings");
            {
                xWMenu.AddItem(
                    new MenuItem("WHitChance", "W 命中率").SetValue(
                        new StringList(new[] { "低", "较低", "高", "很高", "不动的" }, 2)));
                xWMenu.AddItem(new MenuItem("MinWRange", "最小 W 范围").SetValue(new Slider(525 + 65 * 2, 0, 1200)));
                config.AddSubMenu(xWMenu);
            }

            var xEMenu = new Menu("E 设置", "ESettings");
            {
                xEMenu.AddItem(new MenuItem("AutoEI" + Id, "自动-E -> 不动的").SetValue(true));
                xEMenu.AddItem(new MenuItem("AutoES" + Id, "自动-E -> 减速/震惊/传送/陷阱").SetValue(true));
                xEMenu.AddItem(new MenuItem("AutoED" + Id, "自动-E -> 突进的").SetValue(false));
                config.AddSubMenu(xEMenu);
            }

            var xRMenu = new Menu("R 设置", "RSettings");
            {
                xRMenu.AddItem(new MenuItem("MaxRRange" + Id, "最大 R 范围").SetValue(new Slider(1700, 0, 4000)));
                xRMenu.AddItem(new MenuItem("ProtectManaForUlt", "保留蓝量").SetValue(true));
        ...
        {
            config.AddItem(
                new MenuItem("DrawQBound" + Id, "显示Q绑定").SetValue(new Circle(true, System.Drawing.Color.Azure)));
            config.AddItem(
                new MenuItem("DrawW" + Id, "W 范围").SetValue(new Circle(false, System.Drawing.Color.Azure)));
            config.AddItem(
                new MenuItem("DrawE" + Id, "E 范围").SetValue(new Circle(false, System.Drawing.Color.Azure)));

            config.AddItem(new MenuItem("DrawToggleStatus", "显示切换状态").SetValue(true));
